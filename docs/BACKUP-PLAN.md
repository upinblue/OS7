...
| BL5 | ~~**Backup retention and boot-environment retention are one decision and only half is made.**~~ **THE OTHER HALF IS MADE, 2026-09-14** (owner's decision, [VERSIONS-PLAN.md](VERSIONS-PLAN.md) V15/V16). Boot environments now have a retention rule: **the running one and the newest one older than it are never pruned**, and everything beyond that is subject to the same storage-pressure rule as snapshots — 70 % warns, 80 % tightens, 90 % refuses, and nothing is deleted at all unless deleting it could actually reach the target. `Invoke-OS7StorageRelief` is the one that acts, and it still deletes no snapshot itself: it rewrites this policy and sanoid prunes under it, so there is one thinner. `installer/testing/check-storage-logic.py` holds it at 37 checks. **What is still not measured** is what ZFS on this layout actually does at those capacities — the numbers are a policy, not a measurement, and UL9's own half (how many environments an update leaves behind) is unchanged. |
...
| BL9 | ~~**`snapdir` is left at ZFS's default.**~~ **MEASURED ON A MACHINE 2026-09-14, and the assumption holds.** `snapdir=hidden` on `rpool/USERDATA/os7admin_*`, a plain `ls -a` does NOT list `.zfs`, and explicit traversal of `.zfs/snapshot/<snap>/<path>` reads the old contents — including for a file deleted from the live filesystem — **as the unprivileged owner**. `Get-OS7FileVersion` returned four versions of a demo file on that machine, `-DistinctOnly` collapsing the unchanged snapshot. [VERSIONS-PLAN.md](VERSIONS-PLAN.md) §2 M-V4/M-V5/M-V6 and §7b. **What the same run DID find** is that the cmdlet's refusal for a non-ZFS path was unreachable — `/proc/cpuinfo` returned zero versions in silence, because ownership was decided by comparing paths against ZFS mountpoints and `/` is one. Fixed by asking `/proc/self/mountinfo`. |
