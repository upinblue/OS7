...
| C10 | Update sequence | **DECIDED** — §4.2 step 5 gains `autoremove`, step 6 is deleted, and an idempotent migration step is added (§9). **IMPLEMENTED 2026-08-27** in `powershell/OS7/OS7.Update.ps1`, with the order asserted by `installer/testing/check-update-logic.py` — reversing install and autoremove removes the product and both orders "work", so it is checked as an order rather than as a set. Three things §9 left open were settled: a multi-release jump runs EVERY intervening release's migrations in order (C11's reading); the migration contract is `<version>/<chroot\|firstboot>/NN-name`, shipped by `os7-release` and read from there into the descriptor so declared and shipped cannot diverge; and the `firstboot` context exists because BUILD-NOTES #69 makes TPM2 re-enrolment impossible from a chroot, so C10's own example of migration content cannot run where C10 puts it. **Step 6's deletion is conditional and the code says so**: no ISO installs `os7-release` yet, so the branding is verified after the upgrade rather than assumed. [SESSION-UPDATE-TRAIN.md](SESSION-UPDATE-TRAIN.md) |
...
| C13 | `hw-probe` | **DECIDED 2026-08-27 — NOT SHIPPED.** It is in Ubuntu universe (`hw-probe 1.6.5-1build1` in `resolute/universe`, verified) and stays there. It is the only tool this product would carry that transmits a description of the machine to a third party, and a managed image aimed at Intune-controlled fleets should not carry one by default — the arrival of the tool on a machine is then itself a deliberate act somebody took. `Send-OS7HardwareProbe -InstallTool` fetches it, in a prompt separate from the one that authorises the upload, so agreeing to get the tool is never the same gesture as agreeing to send. Nothing else in the product calls it, and `installer/testing/check-device-logic.py` asserts that a `Get-OS7DeviceStatus` run does not invoke `hw-probe` even once — which it did, for `--version`, until that check was written. [SESSION-DEVICE-MANAGER.md](SESSION-DEVICE-MANAGER.md) §5. |

| C14 | Avalonia, and `os7-ui` | **PROPOSED 2026-09-14 — a pinned component, like PowerShell.** OS/7's own GUI applications are Avalonia ([GUI-APPS-PLAN.md](GUI-APPS-PLAN.md) G1). It arrives from NuGet at build time, not from the archive, so by §7's rule it needs a version and a SHA256 in the pin — the same treatment PowerShell and the Foundation Framework get, and for the same reason: a version number is only honest if what it names is fixed (BUILD-NOTES #58). **Measured before proposing** ([SESSION-AVALONIA-FOOTPRINT.md](SESSION-AVALONIA-FOOTPRINT.md)): the feed offered 12.1.2 on 2026-09-14; one application is **22.1 MiB** framework-dependent against **100.8 MiB** self-contained, so C2's runtime is paid back on the second application. **13.3 MiB of that 22.1 is byte-identical in every application** (`libSkiaSharp.so`, `libHarfBuzzSharp.so`), which makes a shared `os7-ui` package a size decision as well as a design one: six applications cost **66.1 MiB** with it and 132.6 MiB without. `os7-ui` must declare `libfontconfig1` and the X11 libraries itself rather than inherit them from `ubuntu-desktop-minimal`. **This is also C3's first consumer**, which is the thing C2 kept the runtime for. |
