...
| `make build-amd64` | **Blocked locally, green in CI, and the medium now BOOTS.** Locally still ENOSYS (§3). On a hosted x86_64 runner it produces an amd64 ISO — 1528 packages, GNOME + Edge + `intune-portal`, `check-image.py` all green — and both jobs of run 32854778263 (2026-08-25) finished green. The first such ISO could not boot at all (no El Torito, no Rock Ridge, [SESSION-AMD64-FIRST-ISO.md](SESSION-AMD64-FIRST-ISO.md)); `build/lib/efi-remaster.sh` fixed that and firmware → GRUB → OS/7's menu is measured on amd64 ([SESSION-AMD64-EFI-REMASTER.md](SESSION-AMD64-EFI-REMASTER.md)). **Nothing past the menu is measured** — no amd64 install, ever. |
