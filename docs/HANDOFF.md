...
| **Versions (Time Machine, in Windows 2000's vocabulary)** | **The READ half is built and has drawn a real file's history on a machine; the write half is built and UNRUN there.** [VERSIONS-PLAN.md](VERSIONS-PLAN.md) is authoritative — §2 measured on a booted machine that every prerequisite already exists and that the snapshots are **already being taken**, so this is a reader, not a second snapshot policy. `Get-OS7FileVersion` lists them (with `-IncludeAbsent`, the row saying the file was not there yet, which is what turns a list into a history), `Restore-OS7File` copies one back, and `os7-versions <path>` is the whole contract with whatever file manager is installed. **The storage answer is the hard part and it is decided and built** (V15/V16, owner 2026-09-14): 70 % warns and deletes nothing, 80 % tightens the retention policy and lets **sanoid** prune under it, 90 % refuses — gated on whether thinning could even reach the target, because destructive and ineffective is the worst outcome available — and the running boot environment and the newest one older than it are never pruned. **And a restore is itself undoable** (V8): `Restore-OS7File` snapshots the live file before it writes over it, on the DESTINATION's dataset, only where something is actually overwritten, saying so in the prompt before it is answered — and OS/7 prunes those itself, because sanoid prunes only what sanoid took (measured). `check-storage-logic.py` is **149 checks** with no ZFS and no VM. **THE FEATURE HAS NOW BEEN USED END TO END ON A MACHINE INSTALLED FROM AN ISO** (`OS7-1.0.0.231-amd64.iso`): right-click a file in GNOME Files → *Versions* → the window draws five versions with the cascade and sanoid's own buckets → pick an older one → *Restore…* → a dialog naming the file and the moment → the file is back, and the work it replaced is beside it as `Angebot.txt.os7-before-restore-20260915-134815`, renamed rather than destroyed because an unprivileged owner may not snapshot (V19, Time Machine's own mechanism). No polkit dialog, no sudo. **Clicking it found three defects in an hour that every check here had passed**: the Nautilus namespace is 4.1 and `gi.require_version` must not be called at all (the menu entry silently vanished); `Select-OS7DistinctVersion` returned `,@($kept)` — the list as ONE object, which the window could not deserialise, while §6 stayed green because member enumeration answered its question correctly; and `-DistinctOnly` collapsed the live row away after a restore, because `rsync -a` preserves mtime, so *Now* disappeared exactly when somebody most needed it. All three fixed, all three now held by checks proven to fire. **And then proven from a medium that carries them**: `OS7-1.0.0.233-amd64.iso`, a second machine, nothing copied on afterwards — context menu, window, restore and the file kept aside, all first try (§2e). Earlier in the day V8/V19 were measured against real ZFS, the storage rule became automatic (`os7-storage-relief.timer` in `os7-backup`, enabled from a fresh install, writing `/run/os7/storage-pressure` for a login banner that asks ZFS nothing), and building it cost **#160**: PowerShell logs every compiled script block to syslog, `os7-backup-replicate` had written 93.7 MB to one bench's journal, and `LogLevel: Error` at `$PSHOME` takes a run from 1.95 MB to 704 bytes. [SESSION-VERSIONS-RESTORE-SAFETY.md](SESSION-VERSIONS-RESTORE-SAFETY.md) |
