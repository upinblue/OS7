...
| **Versions (Time Machine, in Windows 2000's vocabulary)** | **The READ half is built and has drawn a real file's history on a machine; the write half is built and UNRUN there.** [VERSIONS-PLAN.md](VERSIONS-PLAN.md) is authoritative — §2 measured on a booted machine that every prerequisite already exists and that the snapshots are **already being taken**, so this is a reader, not a second snapshot policy. `Get-OS7FileVersion` lists them (with `-IncludeAbsent`, the row saying the file was not there yet, which is what turns a list into a history), `Restore-OS7File` copies one back, and `os7-versions <path>` is the whole contract with whatever file manager is installed. **The storage answer is the hard part and it is decided and built** (V15/V16, owner 2026-09-14): 70 % warns and deletes nothing, 80 % tightens the retention policy and lets **sanoid** prune under it, 90 % refuses — gated on whether thinning could even reach the target, because destructive and ineffective is the worst outcome available — and the running boot environment and the newest one older than it are never pruned. **And a restore is itself undoable** (V8): `Restore-OS7File` snapshots the live file before it writes over it, on the DESTINATION's dataset, only where something is actually overwritten, saying so in the prompt before it is answered — and OS/7 prunes those itself, because sanoid prunes only what sanoid took (measured). `check-storage-logic.py` is **102 checks** with no ZFS and no VM, and **V8 and V19 have run against real ZFS on a machine**: the snapshot holds the bytes the restore destroyed, restoring it gives the work back, seven restores in four seconds hit the one-second name collision three times (#159), and sanoid's 32 snapshots were 32 afterwards. That run also raised the decision the feature owed and the next day settled it: **`zfs snapshot` is root's**, so a file's owner can list every version of it and cannot take the snapshot that makes restoring one undoable. The answer was neither to refuse them nor to grant them the right, but **what Time Machine itself does** — it takes no snapshot before a restore either, it moves the existing file aside ("Keep Both"), and Windows' Previous Versions has the same shape. So a restore keeps what it is about to overwrite by the strongest means available where it stands: a ZFS snapshot with privilege, a rename without — same inode, no bytes, `SafetySnapshot` or `SafetyCopy` in the result, and `-NoSafetyPoint` turns both off. Measured as uid 1000 with no sudo. What is owed: `nautilus-python` in the pinned archive (O-V6), nothing prunes the renamed files (VL9), and no ISO carries any of it. [SESSION-VERSIONS-RESTORE-SAFETY.md](SESSION-VERSIONS-RESTORE-SAFETY.md) |
