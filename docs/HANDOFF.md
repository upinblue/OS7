...
| **Secure Boot: installing from such a medium** | **Never done.** Nothing is known past the welcome screen: no `mokutil --sb-state` from a live session, nothing about kernel **lockdown** (Secure Boot enables it, and an unsigned module then will not load), and no machine has been installed from a Secure-Boot-on medium — so every OS/7 machine in existence is still TPM-sealed against a Secure-Boot-**off** PCR 7. And no harness can do it yet: every one boots the medium with `-kernel`, so the ISO's bootloader never runs (BUILD-NOTES #134). Steps 3 and 4 in [SESSION-SECUREBOOT-MEDIUM.md](SESSION-SECUREBOOT-MEDIUM.md). |
