...
| **GUI applications** | **Two are built; ONE has been SEEN to draw on a machine, and neither has ever completed its job.** `src/OS7.Ui/` is the design system — 17 Windows 2000 colours and a 2px bevel grammar, held equal to the desktop theme's `gtk.css` by `check-gui-tokens.py` because Avalonia cannot read GTK CSS. `src/OS7.App.SoftwareUpdate/` and `src/OS7.App.Versions/` sit on it; `src/OS7.Shell/` is the one channel to PowerShell (G4), and `check-gui-logic.py` holds the capability grants, drives both `--self-test`s in `os7-build:amd64` (57 and 46) and is proven to fire against four planted defects. [GUI-APPS-PLAN.md](GUI-APPS-PLAN.md) G1–G12 is the authority; it cost BUILD-NOTES **#151**, **#152** and **#153**, and the last of those — a global `TextBlock` foreground that beat every inherited value — had shipped in BOTH applications, so every disabled button looked enabled. **What has never happened: an update performed from the Software Update window, and no machine installed from an ISO carrying either package.** |
| **Versions (Time Machine, in Windows 2000's vocabulary)** | **The READ half is built and has drawn a real file's history on a machine; the write half is built and UNRUN there.** [VERSIONS-PLAN.md](VERSIONS-PLAN.md) is authoritative — §2 measured on a booted machine that every prerequisite already exists and that the snapshots are **already being taken**, so this is a reader, not a second snapshot policy. `Get-OS7FileVersion` lists them (with `-IncludeAbsent`, the row saying the file was not there yet, which is what turns a list into a history), `Restore-OS7File` copies one back, and `os7-versions <path>` is the whole contract with whatever file manager is installed. **The storage answer is the hard part and it is decided and built** (V15/V16, owner 2026-09-14): 70 % warns and deletes nothing, 80 % tightens the retention policy and lets **sanoid** prune under it, 90 % refuses — gated on whether thinning could even reach the target, because destructive and ineffective is the worst outcome available — and the running boot environment and the newest one older than it are never pruned. **And a restore is itself undoable** (V8): `Restore-OS7File` snapshots the live file before it writes over it, on the DESTINATION's dataset, only where something is actually overwritten, saying so in the prompt before it is answered — and OS/7 prunes those itself, because sanoid prunes only what sanoid took (measured). `check-storage-logic.py` is **144 checks** with no ZFS and no VM. **V8 and V19 ran against real ZFS on a machine** — the snapshot holds the bytes the restore destroyed, restoring it gives the work back, and the one-second name collision fired three times in four seconds (#159). V19 is the decision that run forced: `zfs snapshot` is root's, so a file's owner can list every version and cannot take the snapshot that makes restoring one undoable. The answer is Time Machine's own — it takes no pre-restore snapshot either, it moves the existing file aside — so a restore keeps what it overwrites by the strongest means available where it stands: a snapshot with privilege, a **rename** without (same inode, no bytes). **And since 2026-09-15 the storage rule is AUTOMATIC**: `os7-storage-relief.timer` in `os7-backup` runs it every fifteen minutes, writes `/run/os7/storage-pressure`, and the login banner reads that file rather than asking ZFS (a banner that asks is a login that hangs on a degraded pool). `Get-OS7RestoreAside` reports what restores left behind and relief removes only the ones a snapshot still holds — never the only copy of somebody's work. **The feature is now packaged**: `os7-app-versions` carries the window and a GNOME Files context-menu entry over `python3-nautilus` (O-V6, measured — the plan asked for `nautilus-python`, which does not exist), and the window gained *Restore…*. On the bench both packages install, dpkg owns their files, apt pulls the binding, and the installed application's self-test passes. **What is owed: the context menu has never been clicked and the Restore button never pressed** — the bench has no graphical session — and no ISO carries any of it. Building it cost **#160**: PowerShell logs every compiled script block to syslog, so `os7-backup-replicate` had written 93.7 MB to one bench's journal; `LogLevel: Error` at `$PSHOME` takes a run from 1.95 MB to 704 bytes, and a malformed file there stops pwsh starting at all. [SESSION-VERSIONS-RESTORE-SAFETY.md](SESSION-VERSIONS-RESTORE-SAFETY.md) |
| **The automation host** | **Phase 1 built, RUN ON A MACHINE and ON AN ISO, 2026-09-14.** `OS7-1.0.0.222-amd64.iso` carries `os7-automation`, and `check-image.py` asks **dpkg** — not the filesystem — who owns `os7-job@.service` and the slice, because only a file a package owns rolls back with the release. (That medium has no repository credential: it was built with `OS7_REPO_NO_CREDENTIAL=1` and its four check failures are all that one fact.) [AUTOMATION-PLAN.md](AUTOMATION-PLAN.md) AU2–AU8 and AU11 in `powershell/OS7/OS7.Automation.ps1` (16 cmdlets), five plumbing verbs in `powershell/Systemd/`, and `build/packages/os7-automation/` — the slice, the templated job unit and the polkit rule. On bench `manual`: a dataset outside the boot environment, a secret sealed against the real TPM, a job that read one JSON document off stdin and its credential out of `$CREDENTIALS_DIRECTORY`, an intent record on disk 1.5 s before the result, and the fence read back out of `systemctl show`. **Two measurements decided the design before a line was written**: `--tpm2-pcrs=` defaults to EMPTY (so `host+tpm2` does NOT inherit #69/#100, and AUL2 shrinks), and systemd's host key lives inside the boot environment (so the sealing target is `tpm2` ALONE — `host+tpm2` would have put the key back where AU6 keeps the secret out of). **Then a machine found three defects while `check-automation-logic.py` was green at 46, `check-layering.py` held seven rules and `check-privilege.py` held both baselines**: a directory the cmdlet's own error message promised and did not create, a property read that throws under strict mode because the job journal has two writers with different fields, and `RuntimeMaxSec=`, which systemd IGNORES for `Type=oneshot` — so every job ran with **no timeout at all** while the unit file looked complete (BUILD-NOTES #155). What is owed: a build that carries the package, a Kerberos ticket from a keytab, one delivered notification, one job under `-Unconfined` and one under `DynamicUser`, one job started through polkit by an unprivileged caller, and arm64. [SESSION-AUTOMATION-PRIMITIVES.md](SESSION-AUTOMATION-PRIMITIVES.md) |
...
./installer/testing/check-device-logic.py     # GREEN  (74 checks, no hardware)
...
- **Should OS/7 become an automation host, and should an identity application be
  built on it?** (Raised 2026-09-14.) **Nothing is decided and nothing is
  built**; what exists is two plans and the boundary between them.
  [AUTOMATION-PLAN.md](AUTOMATION-PLAN.md) is the OS half (AU1–AU14, all
  *Proposed*) and is worth reading even if the application half never happens,
  because it names four gaps this repository measured in itself:
  `Register-OS7ScheduledTask` exposes **no limit, no timeout and no isolation
  parameter**; there is **no secret store, no lock verb and no notification
  verb** anywhere in `powershell/`; **no MTA is in any package list**, so a job
  that fails at 03:00 is known to nobody; and `systemd-creds` — the mechanism
  that cmdlet's own help asks for — is mentioned nowhere in the tree. Those are
  gaps in a machine that already describes itself as a place where scheduled
  work runs, and closing them needs no application above.
  [IAM-PLAN.md](IAM-PLAN.md) is the application half (IG1–IG14) with
  [IAM-SCOPE.md](IAM-SCOPE.md) as its capability scope. Its governing constraint
  is AU1: **the OS provides primitives, the application provides policy**,
  because the alternative is the engine twice in two languages with nobody at
  the seam (#66). Both are design, neither has run, and the measurements each
  owes are listed in their own last section.
