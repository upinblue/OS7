...
| **Remote Desktop (RDP)** | **A CONCEPT, and the mechanism is proven on a machine up to the login screen — no cmdlet exists.** The amd64 GUI image already ships `gnome-remote-desktop` 50.2 (the RDP system daemon, "remote login"), and on the GUI bench a real RDP client (FreeRDP 3.31, and `mstsc` from the Windows host) passed NLA/CredSSP against a machine-wide credential, the daemon started the GDM handover, and **OS/7's own branded login screen was delivered over RDP and photographed** (dossier M-R33/M-R29/M-R14); PAM on that path receives the client's real IP as `rhost` (M-R34). It is **two logins**: a machine-wide RDP credential at the door, then the person's own account at the greeter. What that leaves: **no `OS7.RemoteDesktop.ps1` exists**, a *completed* per-user login over RDP was not landed in the run (owed), the machine credential rests in **plaintext** today (the daemon's user is not in group `tss`, so the TPM path falls back), `Kerberos` is refused by the 50.2 system daemon, and v1 as designed has **no per-user allow-list and no lockout** — so `Enable-` would make a source scope mandatory. amd64-GUI only: headless and arm64 have no daemon (ssh / `Enter-PSSession` is the path). [REMOTE-DESKTOP-PLAN.md](REMOTE-DESKTOP-PLAN.md) is the authority — R1–R17 proposed, RL1–RL13 the honest list, O-R1–O-R18 the measurements owed with their harness. |
