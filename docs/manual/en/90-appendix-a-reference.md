...
| `powershell/OS7/` | the product | 126 |
...
| `Get-NetRadio` | Whether any radio is blocked â€” and whether that question could be answered at all. | *none* |
...
| `Get-ChronyTracking` | What chrony is doing with this clock â€” whether it is disciplined, by whom, and how far off it is. | *none* |
...
| Command | What it does | Parameters |
|---|---|---|
| `Get-SystemdHostName` | The static, transient and pretty host names, from the places that hold them. | *none* |
...
| `Get-OS7AccountLockout` | Whether a wrong password costs anything on this machine, and to whom. | *none* |
...
| `Get-OS7EntraStatus` | Whether this machine can sign a user in with Entra ID â€” and if not, why not. | *none* |
...
| `Get-OS7IntuneEnrollment` | What can be said about Intune on this machine â€” and what cannot. | *none* |
...
| `Get-OS7NetworkConfiguration` | What this machine is CONFIGURED to do about the network, and what it is ACTUALLY doing â€” as two separate answers. | *none* |
| `Get-OS7Release` | What OS/7 releases this machine's channel offers. | Available, Channel, Source |
| `Get-OS7RemoteDesktop` | Whether this machine can be reached with Remote Desktop, and whether it actually would be. | *none* |
...
| `Get-OS7RemoteDesktopUser` | Who may sign in to this machine over Remote Desktop. | *none* |
| `Get-OS7Remoting` | Whether this machine can be reached with PowerShell â€” both ways it can be meant. | *none* |
...
| `Test-OS7RemoteDesktop` | Check the Remote Desktop configuration on this machine, one question at a time. | *none* |
