...
#
# HARDWARE ARRIVED THE SAME WAY, on 2026-09-10, off a branch that predated the
# package route entirely: it carried `stage_ps_module Hardware` against a
# function this file no longer has. The module code now reaches the image only
# through the os7-module .deb, so the merge had nothing to keep except the name
# — and the name had to go in BOTH lists again, which is the second time that
# paragraph above has been the only thing standing between a module and an
# image that silently lacks its fixtures.
for _mod in Zfs Net Time Systemd Directory Hardware OS7; do
