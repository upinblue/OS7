...
build_os7_module() {
	...
	# ALL the generic layers plus OS7, in the order build.sh's staging used to
	# carry and hook 0060 checks. THIS LOOP SAID `Zfs OS7` UNTIL 2026-08-28,
	# and that was a defect rather than a lag: the ISO staged Net, Time and
	# Systemd since each was written, so every ISO-installed machine had them —
	# and a machine installed from OS/7's own repository had two modules, with
	# the failure appearing only when the first network, clock or service
	# cmdlet fell through Import-OS7*Layer to `Import-Module <Name>` and threw
	# about a module the operator never asked for. Nothing looked: pkg_finish's
	# list below named only what the loop already copied, so the two were wrong
	# together. One manifest and one .psm1 per module below is what keeps them
	# honest now.
	for name in Zfs Net Time Systemd OS7; do
		...
		# fixture tree and belongs in the repository, not on a machine. The ISO
		# is the one place that carries the fixtures anyway — build.sh overlays
		# them via includes.chroot so check-image.py can run the self-tests
		# against the artefact, where the subsystems being parsed cannot run.
		rm -rf "${dst}/tests"
