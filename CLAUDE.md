...
./installer/testing/check-storage-logic.py # what happens when the pool fills
                                          ...
                                          #   go red. AND V8 HAS NOW RUN
                                          #   AGAINST REAL ZFS: the snapshot
                                          #   holds the bytes the restore
                                          #   destroyed, restoring it gives the
                                          #   work back, and seven restores in
                                          #   four seconds hit the one-second
                                          #   name collision THREE times
                                          #   (#159). That run found what no
                                          #   fake could — `zfs snapshot` is
                                          #   root's, so the OWNER of a file
                                          #   can list every version of it and
                                          #   cannot take the snapshot that
                                          #   makes restoring one undoable.
                                          #   Case I is that refusal, and every
                                          #   clause of it is held by one
                                          #   assertion that fires alone. V19
                                          #   is the answer to that, and it is
                                          #   TIME MACHINE'S OWN: Apple takes no
                                          #   snapshot before a restore either —
                                          #   it moves the existing file aside
                                          #   ("Keep Both") — so OS/7 keeps what
                                          #   it is about to overwrite by the
                                          #   strongest means available where it
                                          #   stands. A snapshot with privilege,
                                          #   a RENAME without: same inode, no
                                          #   bytes, and the owner is never
                                          #   refused their own file. Eight
                                          #   planted defects go red, one per
                                          #   rule. 102 checks, no ZFS, seconds
...
./installer/testing/check-layering.py     # Z1, P2, P2-time, P2-systemd,
                                          #   P2-directory, P2-hardware: does
                                          #   OS7 still reach ZFS, the network,
                                          #   the clock, systemd, the directory
                                          #   or the hardware directly. SIX
                                          #   rules, measured at
                                          #   0 / 0 / 0 / 2 / 1 / 0 against
                                          #   baselines that may fall and may
                                          #   not rise; each non-zero site is
                                          #   NAMED. This entry said FOUR and
                                          #   listed four until 2026-09-09,
                                          #   while the prose forty lines
                                          #   further down said five - the file
                                          #   disagreeing with itself about a
                                          #   number the check prints
...
**Seven PowerShell modules since 2026-09-10, and the direction between them
...
pattern to catch that shape is what found it. It held at 0 again when the device
manager was merged forward on 2026-09-10, which is the first time one of these
rules was asked about 78 commits of code written after it.
...
Its MEDIUM is no longer behind, though: `OS7-1.0.0.194-arm64.iso` carries
the signed chain and `check-image.py arm64` passes on it, both produced on the
Windows box under emulation (#140). What arm64 owes is a BOOT, which needs
HVF and therefore the Mac.
