...
1. **Pin the download.** Fetched and hashed on 2026-08-22; assert before use,
   ...
   together.
2. **Subset to EXACTLY 256 or 512 glyphs — not "at most".** This was written
   here as a cap for months and it is not one: `fbcon_set_font()` rejects any
   other count with `-EINVAL`, which `setfont` reports as
   `ioctl(KDFONTOP): Invalid argument`. A 441-glyph PSF is well-formed and
   unloadable (BUILD-NOTES #59). `bdf2psf` is told 512 and pads, which is why
   this route never noticed; `psf.py verify` now asserts it for both fonts. The
   font
...
point `verify` correctly reports the absence as a note. BUILD-NOTES #57.
...
fix an output when something unpinned sits between them. BUILD-NOTES #58.
...
> **`<user>` is the ACCOUNT'S name, and for two months it was not (L32).**
> `New-OS7Storage -UserName` defaulted to `os7` and the storage step never
> passed it, so the drawing above was right and the machine was not: `/home/os7`
> was an empty dataset and the real home was an ordinary directory inside
> `rpool/ROOT/<be>`. The layout is only as good as the name it is given.
> Since 2026-08-26 the executor passes `plan.Account.Username`, the default is
> gone, and `AccountStep` refuses to finish an install whose home is on the same
> filesystem as `/`. [../docs/BUILD-NOTES.md](../docs/BUILD-NOTES.md) #74 and
> #78; the migration for machines already installed is `Move-OS7Home`.
...
rollback logic. **Setup needs the identical logic** to create the first boot
environment. Writing it twice guarantees drift.
...
| L9 | Console fonts hold 256 or 512 glyphs — not "up to" (BUILD-NOTES #59); no CJK, no RTL | English + German for v1; state the limit rather than pretending |
...
| L16 | Intune's "Allowed distributions" rule matches on `/etc/os-release`; branding OS/7 as its own `ID=` could make every device fail it | keep `ID=ubuntu` / `ID_LIKE=ubuntu` / `VERSION_ID="26.04"`, brand only `NAME` / `PRETTY_NAME` (§4.6). **RESOLVED 2026-08-23 (D8):** the product identity moves to `IMAGE_ID` / `IMAGE_VERSION`, so OS/7 is identifiable as itself without touching a field Intune matches on. **REOPENED AND RE-RESOLVED 2026-08-26:** "a field Intune matches on" was a guess, and it was a guess about one vendor tool out of three. Measured (BUILD-NOTES #80): Arc's onboarding script keys on `NAME`, `intune-agent` reads `ID` / `VERSION` / `VERSION_ID` / `PRETTY_NAME`, and no agent carries a distribution allowlist at all — it reports `OSDistribution` / `OSVersion` and the *service* decides, so the deciding value cannot be observed from the device. The resolution is no longer "protect the right field" but "**do not let the brand depend on any of them**" ([../docs/IDENTITY-PLAN.md](../docs/IDENTITY-PLAN.md) I1). **Still unverified end to end: no OS/7 device has ever enrolled** — IL2 there |
...
| L19 | A PSF holds exactly 256 or 512 glyph slots; Fixedsys Excelsior has 6 192 codepoints | **RESOLVED by S1 (2026-08-24), and it was never close to binding.** 409 codepoints requested, 434 mapped into 512 positions, every required block complete (ASCII 95/95, Latin-1 96/96, Box Drawing 128/128, Block Elements 32/32). Latin Extended-A turned out unnecessary: German is entirely inside Latin-1, so only the cp1252 extras are carried. **The real risks were not the count** — `bdf2psf`'s stock equivalences silently replaced the double-line box with the single-line one, and the font draws 15 px of ink in a 16-px cell so every vertical border came out dashed. Both are fixed in the pipeline and both are now asserted; neither was visible in a coverage count. BUILD-NOTES #26 and #27 |
...
| L32 | **THE LAYOUT IN §4.4 WAS RIGHT AND THE MACHINE WAS NOT, for two months, because of one argument.** `New-OS7Storage` creates `rpool/USERDATA/<UserName>_<suffix>` at `/home/<UserName>` and its `-UserName` defaulted to `os7`; the storage step built its command with four arguments and not five, and the account is created six steps later by `useradd -m` under whatever the operator typed. So every machine this repository has installed has an empty dataset at `/home/os7` and the account's real home an ordinary directory inside `rpool/ROOT/<be>` — which `Restore-OS7` rolls back with the system, the one outcome §4.4 exists to prevent. Nothing saw it: `grep -rIn "/home" installer/testing/*.py` returned nothing at all, so the harness that installs a machine, boots it with no ISO attached and logs into it had never asked where that account's FILES were. A second finding came out of fixing it and is worse in its way: with the dataset mounted first, **`useradd -m` warns, exits 0, copies no `/etc/skel` and changes no ownership** (BUILD-NOTES #78, measured on this image's own `passwd`), so passing the argument alone yields a correctly-placed home that is `root:root`, empty, and unwritable by its owner | `StorageSteps` passes `plan.Account.Username` — available because the executor's contract is a complete plan, validated at `ExecuteScreen.Start` and at `--unattend` and nowhere else. The `os7` default is **removed**: empty now means no home dataset at all, which is the honest answer for `--storage-only`, and the result object reports `userName`/`userDataset` into the install log. `AccountStep` copies `/etc/skel` into an empty home, chowns it, chmods it to the `HOME_MODE` it READS out of `/etc/login.defs`, and then asks `stat` what it did — failing the install if `/home/<user>` shares a device number with `/`. `run-phase3.py` checks 9 and 10 make the same claim about a machine that has booted, from `findmnt` and from `stat -c %d` independently; `run-s5.py` asserts a file written in a clone survives the rollback that removes the clone's package. Machines already installed are migrated by `Move-OS7Home`, checked without a VM by `installer/testing/check-home-logic.py`. **Not verified: `run-phase3.py all` has not been run since the change** |
| L33 | **THE SETUP MEDIUM IS A WORKSTATION UNTIL IT IS TOLD OTHERWISE, AND AN INSTALL CAN LOSE A RACE WITH IT FOR MEMORY.** Reported on 2026-08-26 from a 6 GB amd64 VM: at "Creating the ZFS pools and datasets" the kernel killed `networkd-dispatcher` and `unattended-upgrades` and then blocked `systemd:1` for 122 seconds. Neither victim is Setup's — they are the shipped image's own enabled units. Read out of the squashfs afterwards, the Install entry's `multi-user.target` pulls in **39** units and **15** timers, among them `unattended-upgrades`, six `snapd` units, `packagekit`, `cups`, `sssd` and `apt-daily.timer` with `APT::Periodic::Update-Package-Lists "1"` — an `apt update` downloading into casper's tmpfs overlay, which is RAM, on a medium with no swap, while `zfs_arc_max` sits at its default of half of physical memory because the image ships no `modprobe.d` options for it at all. `systemd.unit=multi-user.target` was put on that command line to keep gdm3 off tty1 (BUILD-NOTES #49) and does exactly that; it never made the medium an installer | Two halves, neither of which touches the live entries — "try before you install" (L14) still boots a desktop with its snapd and its cron. `/usr/lib/systemd/system-generators/os7-setup-quiesce` masks **62** units when, and only when, `os7.setup=1` is on the kernel command line; being a *generator* it runs before systemd builds its first transaction, so nothing is started and then stopped. `InstallerEnvironmentStep` — the new step 0 of `StorageSteps.For`, so `--storage-only` gets it too — writes `min(MemTotal/8, 1 GiB)` to `/sys/module/zfs/parameters/zfs_arc_max` and **reads it back**, and logs MemTotal/MemAvailable/Shmem/SwapTotal. Hook 0070 runs the generator's gate against four command lines at build time, including `noos7.setup=10` — the substring that would take cron away from a live session. **THE FLOOR IS STILL NOT MEASURED**: the step *warns* below 4 GiB rather than refusing, because refusing on a number nobody has established would stop installs that work. BUILD-NOTES #79, [../docs/SESSION-INSTALLER-MEMORY.md](../docs/SESSION-INSTALLER-MEMORY.md). **Not verified: no ISO has been rebuilt and no install has been run since the change** |
| L34 | **THE KERNEL WRITES OVER SETUP'S SCREEN, and the command line does not stop it.** The Install entry carries `quiet loglevel=0` and the comment beside it said that was the kernel dealt with. The image overrules it: `/usr/lib/sysctl.d/55-console-messages.conf` sets `kernel.printk = 4 4 1 7`, applied by `systemd-sysctl` during boot, so `console_loglevel` is back to 4 before the first frame is painted and everything at KERN_ERR or above lands on top of the installer — four lines per OOM victim, three per blocked task. It is why L33's failure was legible only as a photograph of a corrupted screen. Third instance of the same shape after #25 and #62: a kernel parameter is a request, and the image gets the last word | `Terminal.QuietTheKernel` takes `console_loglevel` to 1 while Setup owns the console and puts the original back via `Restore()` — the same ProcessExit/SIGINT/SIGTERM/SIGHUP guarantee as raw mode and the palette — reading the file back after writing it. Nothing is lost: the ring buffer is untouched, `dmesg` on tty2 still has every line, and `Diagnostics.KernelLog` copies the kernel's error-and-worse lines into Setup's own log on a failure, which is where they were wanted. The command line keeps `loglevel=0`, which covers the interval between the kernel starting and `systemd-sysctl` running. BUILD-NOTES #79 |
| L35 | **SCREEN 9D CAN JOIN A DOMAIN AND NO MACHINE HAS EVER LET IT.** The screen is drawn, `DomainStep` runs between `NetworkStep` and `InstallLogStep`, and the whole path is exercised only against a Samba AD domain controller in a container (`installer/testing/check-ad.py`). Three narrower gaps sit inside that one. **First**, the join happens from the installer's environment against the TARGET's `/etc`, so it depends on the live network screen 9 brought up (D12) rather than on the installed machine's — a medium that can reach the domain controller and a machine that cannot afterwards would produce a keytab on a host that cannot use it, and nothing checks for that. **Second**, screen 12 prints the disk, the account and the network and says nothing about the domain, so an operator whose join failed learns it at the first login attempt rather than at the end of Setup. **Third**, the credential reaches `adcli` on stdin and is never written down, which is right (P7) and means a join that fails cannot be retried without typing it again — with ONE exception, added 2026-09-07: when the failure is the one a machine behind NAT gets, `DomainStep` retries once by itself. adcli's default sets the computer password through the Kerberos set-password service, whose exchange is integrity-protected over the addresses each end sees, so behind NAT it fails with "Message stream modified"; `Join-OS7Domain -UseLdapPassword` sets it with an LDAP modify instead and the same join succeeds (measured against Windows Server 2025, [../docs/SESSION-AD-JOIN.md](../docs/SESSION-AD-JOIN.md)). Screen 9D does NOT ask about it — nobody in front of a text-mode installer knows whether this machine reaches that domain controller through NAT, and a field whose question cannot be answered where it is asked is worse than none — so the retry is driven by the CMDLET'S OWN failure message naming the switch, `DomainPlan.UseLdapPassword` records which road the join took, and `--unattend` can set it up front. `--self-test` asserts all three: it retries when told, it does not retry a wrong password, and it never retries twice. | The packages are on the medium since 2026-08-28 (`adcli`, `sssd`, `sssd-ad`, `sssd-tools`, `krb5-user` in `os7-base.list.chroot`), so the screen no longer skips itself on every install — which is what it did for the whole first day it existed. `DomainStep` reads the keytab back with `klist -k` and asks the name service to resolve an account rather than trusting `adcli`'s exit code, and `Test-OS7Domain` re-asks the same questions on a running machine. The screen-12 line and the first-boot verification are owed; the gate is an ISO build and an install, which is `run-phase3.py`, which needs the Apple Silicon host. [../docs/AD-PLAN.md](../docs/AD-PLAN.md) AL1 |
...
**Taken as of 2026-08-28: D1–D16, L1–L35.** The next free numbers are **D17** and
**L36**. If you take one, move this line in the same commit. The same rule and
the same table live at the top of [../docs/BUILD-NOTES.md](../docs/BUILD-NOTES.md)
for its numbers.

**This line was wrong for three days, and that is worth leaving written down.**
Until 2026-08-28 it said `L32` was free while `L32`, `L33` and `L34` were already
rows in the table above — three limitations, three sessions, each of which wrote
its entry and none of which moved the counter. The rule above says a number is
claimed where the claim survives `git pull` — and this counter is in that same
file, moved by that same commit, so a counter nobody moves is a claim nobody can
see and a free number that is not free. Checking it costs a second:
`grep -c '^| L3[0-9] |' installer/SETUP-PLAN.md` against what this sentence says.
...
| D16 | Does Setup join an Active Directory domain, and if so, when | **DECIDED 2026-08-27 — yes, on screen 9D, DURING THE INSTALL.** Three decisions in one, and the timing is the one that needed an argument. It is the opposite of the TPM case (#69), where enrolment had to move to first boot, and the reason does not transfer: TPM sealing depends on PCR values that genuinely differ between the installer and the installed system, while a join depends on the hostname, DNS, the clock and a credential, which are the same in both. Joining on first boot would therefore buy nothing and cost the one thing that matters — it would mean **persisting a domain credential between the install and the reboot**, which P7 forbids outright. So the credential is collected on 9D, used by `DomainStep`, and never touches disk. Second: **`adcli`, not `realmd`** — realmd is a D-Bus service that expects to configure a running system and the installer works against a chroot, while adcli is a one-shot program that performs the join and writes the keytab; OS/7 writes `sssd.conf` itself because its contents are policy (AD-PLAN A9, the home directory). Third: the **preferred** credential is a pre-created computer account with a one-time password, so that a fleet never types a domain administrator's password into a text-mode installer at all. The screen offers both and says which is which. Reasoning and the measurements behind it: [../docs/AD-PLAN.md](../docs/AD-PLAN.md) A7, A8 |
...
`D16` and `L35` are the **domain join** — screen 9D — and both rows are in the tables
above as of 2026-08-28. They were claimed by three source files
(`Model/DomainPlan.cs`, `Screens/DomainScreen.cs`, `Steps/DomainSteps.cs` each cite
`D16, L35` in their headers) a day before either row was written, which is the same
failure the counter above records from the other direction: a number cited by code
and absent from the table is a decision nobody can read.
...
| D8 | `/etc/os-release` identity: brand it as OS/7, or stay `ID=ubuntu` for Intune | **CLOSED 2026-08-23, and without a trade-off.** os-release has fields for exactly this: `IMAGE_ID=os7` + `IMAGE_VERSION=<version>` carry the product identity while `ID=ubuntu` / `ID_LIKE=ubuntu` / `VERSION_ID="26.04"` stay untouched for Intune, and `NAME` / `PRETTY_NAME` / `HOME_URL` are branded as already proposed. Note `BUILD_ID` is the **wrong** field — systemd defines it as the original installation base, which by design does not move during updates. Details and the systemd citation: [../docs/RELEASE-AND-UPDATE-PLAN.md](../docs/RELEASE-AND-UPDATE-PLAN.md) §3.5. One caveat inherited: `/etc/os-release` is a conffile of `base-files`, so the branding must be re-asserted idempotently after every update, not written once at install. **CORRECTED 2026-08-26 — `NAME` GOES BACK TO `Ubuntu`.** The reasoning above rests on "Intune matches on `ID`", and that was never verified against Microsoft's code. It is wrong for at least one consumer: the Azure Arc onboarding script reads **`NAME`**, matches `*buntu*`, and exits 133 without ever reading `ID`; and `intune-agent` reads `ID`, `VERSION`, `VERSION_ID` **and** `PRETTY_NAME`. Three Microsoft consumers, three field sets (BUILD-NOTES #80). D8's conclusion — the identity can be carried without a trade-off — still holds; the field list does not. The corrected list, and the decision that the brand must stop depending on any single os-release field, are [../docs/IDENTITY-PLAN.md](../docs/IDENTITY-PLAN.md) §4 and I1–I4, which is now authoritative for this row |
...
`Update-OS7`/`Restore-OS7` call it.

**The module half is DONE, 2026-08-25.** `Get-`/`New-`/`Set-`/
`Remove-OS7BootEnvironment` and a real `Restore-OS7` are in `powershell/OS7/`,
and spike S5 walked clone → change → activate → reboot → roll back on a machine
installed by Setup — [../docs/SESSION-BOOT-ENVIRONMENTS.md](../docs/SESSION-BOOT-ENVIRONMENTS.md).
What it changes for this phase: **activation is not a ZFS operation**, so
...
| `powershell/OS7/OS7.psm1` | **DONE 2026-08-25.** The BE primitives Setup and `Update-OS7` share, as a *pair* of datasets (§6.3), plus a real `Restore-OS7` using §4.4's naming scheme. What activation turned out to be — an ESP file and `saved_entry`, not a ZFS property — is in [../docs/SESSION-BOOT-ENVIRONMENTS.md](../docs/SESSION-BOOT-ENVIRONMENTS.md). The full cmdlet surface is in [../docs/RELEASE-AND-UPDATE-PLAN.md](../docs/RELEASE-AND-UPDATE-PLAN.md) §6. |
