...
| Updates / release train | not started — **designed**: [docs/RELEASE-AND-UPDATE-PLAN.md](docs/RELEASE-AND-UPDATE-PLAN.md). Version scheme, product identity and archive pinning are **locked** (U2–U4); the update mechanism (U1) and cadence (U5) are recommendations, and U6 / U8 are open questions 5 and 6 below |
...
- **Updates:** curated release train over ZFS boot environments, driven by the OS7 PowerShell module: `Set-OS7Mode`, `Update-OS7`, `Restore-OS7`. Specified in full — mechanics, versioning, drift, cadence — in [docs/RELEASE-AND-UPDATE-PLAN.md](docs/RELEASE-AND-UPDATE-PLAN.md). What is settled and what is not:
  - **Ubuntu security patches within 26.04 need no OS/7 code change.** OS/7 is real Ubuntu; what changes is the *delivery* (into a cloned boot environment), not the source. Four things do break under ordinary patches, all of them consequences of OS/7's own design — see the plan §2.2, and Open question 6 below for the worst of them.
...
5. **Is `/var` inside the boot environment?** (U6, raised 2026-08-23 by [docs/RELEASE-AND-UPDATE-PLAN.md](docs/RELEASE-AND-UPDATE-PLAN.md) §4.4.) [installer/SETUP-PLAN.md](installer/SETUP-PLAN.md) §4.4 makes `var` a child of the boot environment, which is correct for install and wrong for rollback: **rolling back a bad update would also roll back `/var/lib/<service>`** — a database, a container store — on the headless product, which is the server target. `USERDATA` is correctly outside `ROOT`; workload state is not. Package state (`/var/lib/dpkg`, `/var/lib/apt`) must stay *inside* the BE or it desynchronises from the root it describes, so this is a split, not a move. **Decide before `os7-setup` writes its first dataset layout — like `USERDATA`, it cannot be retrofitted onto installed machines.**
6. **What happens to a fleet when Secure Boot policy changes?** (U8, raised 2026-08-23 by the same document; the operational face of SETUP-PLAN L17.) TPM2 auto-unlock seals against PCR 7. A `shim` or `dbx` update — an entirely routine Ubuntu patch — invalidates it and drops every machine back to a passphrase prompt that nobody in a managed fleet has. There is **no recovery story**, and no proposal beyond escrowing a recovery key at enrolment. Blocking for fleet deployment; not blocking for the spikes.
