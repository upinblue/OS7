...
The arm64 ISO builds and boots into OS/7 Setup, in OS/7's own colours and console font, and **Setup installs a machine that boots** — partitions, ESP, LUKS2, both pools, the dataset layout, the system, an administrator account, the initramfs and the bootloader. Verified by starting the installed disk with no setup medium attached. That a *finished* OS/7 boots from such a disk under Secure Boot with TPM2 auto-unlock was proven separately by spikes S3, S4 and S6, from a hand-written sequence rather than from the installer. **amd64 now has an ISO and it cannot boot** — built by CI on 2026-08-25, 1528 packages carrying GNOME, Edge and `intune-portal`, every check in `check-image.py` green, and no boot path on the medium at all ([docs/SESSION-AMD64-FIRST-ISO.md](docs/SESSION-AMD64-FIRST-ISO.md)). Nothing in Phases 1–3 is an amd64 result. The *interactive* walk reaches the Complete screen as of 2026-08-25 — it did not for the commit before, and three automated paths passed throughout ([docs/SESSION-SCREEN6-GATE.md](docs/SESSION-SCREEN6-GATE.md)); `./installer/testing/run-phase3.py walk` is what now installs a machine by keypress alone. Read this table before assuming any command in this repo actually works.
...
| CI (`.github/workflows`) | **green on both architectures, and the only way an amd64 ISO can be built at all.** [Run 32838033581](https://github.com/upinblue/OS7/actions/runs/32838033581) (2026-08-25): arm64 14m32s, amd64 25m53s, both uploading an ISO. It took four dispatches and four stale live-build stages to get there ([docs/BUILD-NOTES.md](docs/BUILD-NOTES.md) #44, #47). Trigger stays `workflow_dispatch`: a green build of an ISO that cannot boot is not a signal to put on every push |
...
| Console font (Fixedsys Excelsior → PSF) | **built, asserted and shipped** — `build/lib/build-console-font.sh` converts the pinned TTF at build time and fails the build if a glyph the UI draws is missing, blank, or has been collapsed onto another shape. Setup's font; since D15 it is no longer the installed console's |
| Installed console font (Cascadia Mono → PSF) | **designed and proven against the artefact; not in the build.** Both PSFs were produced and pass `psf.py verify` with no failures and no notes, from the font already in the pinned snapshot — but nothing in `build/` produces them yet and no console has run them. The route is specified in [installer/SETUP-PLAN.md](installer/SETUP-PLAN.md) §2.8; it is a second route, because `otf2bdf` cannot reach an 8×16 cell from this font ([docs/BUILD-NOTES.md](docs/BUILD-NOTES.md) #52). Evidence: [docs/SESSION-CASCADIA-CONSOLE.md](docs/SESSION-CASCADIA-CONSOLE.md) |
| Bootable ISO | **arm64: builds, boots and installs.** ~2 GB, and it carries OS/7 — PowerShell, the `OS7` module, `os7-setup`, the console font and a branded `/etc/os-release` — behind two GRUB entries, *Install OS/7* and a plain live session. What it installs boots from the disk with no ISO attached: [docs/SESSION-PHASE3-SYSTEM.md](docs/SESSION-PHASE3-SYSTEM.md). Ask a built one what it is in seconds with `./installer/testing/check-image.py`. **amd64: builds in CI, and cannot boot.** `OS7-1.0.0.45-amd64.iso`, 3196 MB, 1528 packages, every `check-image.py` check green including `os7-setup --self-test` run inside the image — but live-build writes no El Torito entry and no Rock Ridge for it, so the medium has no boot path. It needs the sibling of `build/lib/efi-remaster.sh`, which is not written: [docs/SESSION-AMD64-FIRST-ISO.md](docs/SESSION-AMD64-FIRST-ISO.md). Locally amd64 is still blocked ([docs/BUILD-NOTES.md](docs/BUILD-NOTES.md) #12) |
...
- **Installed console font: [Cascadia Mono](https://github.com/microsoft/cascadia-code) 2407.24** (decided 2026-08-25, D15) — the default font for the **installed system in non-GUI mode**. Not for GNOME, and not for `os7-setup`, which keeps Fixedsys. The split is the argument: Setup reproduces MS-DOS 6.22 and Windows 2000 and needs the period face; the installed console is where an admin works in PowerShell, and Cascadia is what Microsoft ships for that as the Windows Terminal default.
  - **Measured before it was decided** — [docs/SESSION-CASCADIA-CONSOLE.md](docs/SESSION-CASCADIA-CONSOLE.md). All 356 codepoints the UI requires are present and uniform-width; of the optional set only `U+21B5` is missing, against nine missing in Fixedsys. Both built PSFs pass the repo's own `psf.py verify` with **no failures and no notes** — which the shipping Fixedsys PSFs do not manage.
  ...
  - **Two things it costs.** It is **OFL 1.1**, not CC0: the licence text has to ship beside the PSFs, and because a TTF→PSF conversion is a "Modified Version" under that licence, the files are named `os7-console-*.psf` rather than after the font (Reserved Font Name "Cascadia Code"). And it needs a **second build route** — `otf2bdf` scales both axes together, so from this font it can only produce 8×15 or 9×16, never 8×16 ([docs/BUILD-NOTES.md](docs/BUILD-NOTES.md) #52). Rasterising straight to the cell fixes that and pays a bonus: 16×32 becomes a real rasterisation instead of Fixedsys's pixel doubling, and 16×32 is the size that ships.
  - **Not yet booted.** The font, the conversion and the resulting PSFs are verified; a console actually running them is not. Closing that is a `vmscreen` run, the same way spike S1 closed it for Fixedsys.
