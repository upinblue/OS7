...
| CI (`.github/workflows`) | **builds arm64; amd64 still fails.** [Run 32830869552](https://github.com/upinblue/OS7/actions/runs/32830869552) (2026-08-25, first dispatch of this file): **arm64 green** in 15m57s on the free native `ubuntu-24.04-arm` runner, `OS7-1.0.0.42-arm64.iso` uploaded — an OS/7 ISO now builds somewhere other than one Mac. **amd64 red** in 23m2s, in live-build's `lb_binary_syslinux`, one stage further than the June attempt got ([docs/BUILD-NOTES.md](docs/BUILD-NOTES.md) #47). Trigger stays `workflow_dispatch` until amd64 finishes |
