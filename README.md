...
The arm64 ISO builds and boots into OS/7 Setup, in OS/7's own colours and console font, and **Setup installs a machine that boots** — partitions, ESP, LUKS2, both pools, the dataset layout, the system, an administrator account, the initramfs and the bootloader. Verified by starting the installed disk with no setup medium attached. That a *finished* OS/7 boots from such a disk under Secure Boot with TPM2 auto-unlock was proven separately by spikes S3, S4 and S6, from a hand-written sequence rather than from the installer. **amd64 is unvalidated** — no amd64 ISO has ever been built, and the GUI path is amd64-only. And at this commit the *interactive* walk stops at screen 6: the confirmation validates the whole plan, including the account screen 7 has not asked for yet ([ConfirmScreen.cs](installer/src/OS7.Setup/Screens/ConfirmScreen.cs) → [InstallPlan.cs](installer/src/OS7.Setup/Model/InstallPlan.cs)); `--unattend` and the automated Phase 3 run pass because neither takes that path. Read this table before assuming any command in this repo actually works.
