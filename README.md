...
The arm64 ISO builds and boots into OS/7 Setup, in OS/7's own colours and console font, and Setup **prepares a ZFS-on-LUKS disk** — partitions, ESP, LUKS2, both pools and the dataset layout. It does not yet copy a system onto it; that is Phase 3. That a *finished* OS/7 boots from such a disk under Secure Boot with TPM2 auto-unlock was proven separately by spikes S3, S4 and S6, from a hand-written sequence rather than from the installer. `os7-setup` itself is not written yet, and amd64 is unvalidated. Read this table before assuming any command in this repo actually works.
...
| Updates / release train | not started — **designed**: [docs/RELEASE-AND-UPDATE-PLAN.md](docs/RELEASE-AND-UPDATE-PLAN.md). Version scheme, product identity, archive pinning and the `/var` split are **locked** (U2–U4, U6); the update mechanism (U1) and cadence (U5) are recommendations; U8 is open question 7 below |
| Installer (`os7-setup`) | **Phases 1 and 2 done** — runs on arm64 from the ISO's *Install OS/7* entry and **installs the storage layout**: partition table, ESP, LUKS2 container, both ZFS pools and the §4.4 datasets, with `--unattend` and `--dry-run`. 3.6 MB NativeAOT, no .NET runtime at run time. The result does not boot yet — that is Phase 3: [docs/SESSION-PHASE1-SETUP.md](docs/SESSION-PHASE1-SETUP.md), [docs/SESSION-PHASE2-STORAGE.md](docs/SESSION-PHASE2-STORAGE.md) |
