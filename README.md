...
| CI (`.github/workflows`) | **never run in this history — and its predecessor ran once, in June, with results nobody here had read.** [Run 28103636078](https://github.com/upinblue/OS7/actions/runs/28103636078) (2026-06-24, from the history commit `e1a63f9` harvested): **arm64 succeeded** on `ubuntu-24.04-arm` in 15m2s and uploaded an ISO — the free native runner label is real; **amd64 failed on a native x86_64 runner**, but only after debootstrap and the whole chroot, in live-build's `lb_binary_memtest` ([docs/BUILD-NOTES.md](docs/BUILD-NOTES.md) #44). So amd64's honest status is *got through the rootfs and fell over assembling the medium*, not *never tried*. The current file has never been dispatched |
